#!/usr/bin/env python3
"""Create representative fixed-window token calibration data for Qwen2.5."""
...
from qwen2_tokenizer import DEFAULT_SYSTEM, render_chat
...
def default_pad_token(model_dir: Path) -> int:
    ...
    if generation_config.exists():
        ...
        if "pad_token_id" in data:
            return int(data["pad_token_id"])
    config = json.loads((model_dir / "config.json").read_text(encoding="utf-8"))
    return int(config.get("bos_token_id", 151643))
...
def main() -> None:
    ...
    parser.add_argument("--pad-token", type=int)
    args = parser.parse_args()

    tokenizer = Tokenizer.from_file(str(args.model_dir / "tokenizer.json"))
    pad_token = default_pad_token(args.model_dir) if args.pad_token is None else args.pad_token
    ...
    for index, text in enumerate(RAW_PROMPTS):
        ...
        np.save(args.output_dir / filename, fixed_window(ids, args.seq_len, pad_token))
